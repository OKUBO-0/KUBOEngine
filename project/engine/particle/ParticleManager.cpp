...
void ParticleManager::Initialize(DirectXCommon* dxcommn, SrvManager* srvmaneger, CameraManager* cameraManager, ModelManager* modelManager, TextureManager* textureManager)
{
...
void ParticleManager::Update()
{
	...
	for (auto& [name, particleGroup] : particleGroups) {
		...
		if (!behavior) {
			Log("ParticleManager::Update skipped. Behavior is not set for group: " + name + "\n");
			continue;
		}
		uint32_t counter = 0;
...
{
	if (!particleGroups.contains(name)) {
		Log("ParticleManager::Emit failed. Particle group not found: " + name + "\n");
		return;
	}
	if (!particleGroups.at(name).behavior) {
		Log("ParticleManager::Emit failed. Behavior is not set for group: " + name + "\n");
		return;
	}
...
void ParticleManager::SetBehavior(const std::string& groupName, std::unique_ptr<IParticleBehavior> behavior)
{
	if (!particleGroups.contains(groupName)) {
		Log("ParticleManager::SetBehavior failed. Particle group not found: " + groupName + "\n");
		return;
	}
